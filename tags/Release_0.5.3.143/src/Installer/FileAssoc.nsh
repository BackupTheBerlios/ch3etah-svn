...
;
; Never insert the APP_ASSOCIATE macro multiple times for a single file type,
; it is only meant to associate an application with a single file and using the
...
;   !insertmacro APP_ASSOCIATE "txt" "myapp.textfile" "File Description" "$INSTDIR\myapp.exe,0" \
;     "Open with myapp" "$INSTDIR\myapp.exe $\"%1$\""
