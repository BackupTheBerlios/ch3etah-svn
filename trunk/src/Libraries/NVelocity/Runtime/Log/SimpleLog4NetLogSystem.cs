...
namespace NVelocity.Runtime.Log {
	...
	/// </summary>
		protected log4net.Core.ILogger logger = null;
//		protected log4net.Core.ILogger logger = null;
	...
	private void  internalInit(String logfile) {
	    ...
	    if (o is log4net.Core.ILogger) {
		this.logger = (log4net.Core.ILogger)domain.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.FullName);
	    } else {
	...
	public virtual void LogVelocityMessage(int level, String message) {
		...
		switch (level) {
			...
			logger.Log(this.GetType(), Level.Warn, message, null);
			break;

		    case NVelocity.Runtime.Log.LogSystem_Fields.INFO_ID:
			logger.Log(this.GetType(), Level.Info, message, null);
			break;

		    case NVelocity.Runtime.Log.LogSystem_Fields.DEBUG_ID:
			logger.Log(this.GetType(), Level.Debug, message, null);
			break;

		    case NVelocity.Runtime.Log.LogSystem_Fields.ERROR_ID:
			logger.Log(this.GetType(), Level.Error, message, null);
			break;
		    default:
			logger.Log(this.GetType(), Level.Debug, message, null);
			break;
